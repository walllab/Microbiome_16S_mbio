...
ps.clos<-subset_taxa(ps, Family=="f__Pasteurellaceae")
...
tree_analysis<-plot_tree(ps.clos,color="enriched", label.tips = "Genus.y", title = "Family Pasteurellaceae")

node_of_interets<-tree_analysis$data$V1[tree_analysis$data$OTU == rownames(final_results)[final_results$ESV == "ESV#6"]]
...
to_extract_with_ESV <-tmp[tmp != rownames(final_results)[final_results$ESV == "ESV#6"]]
...
#STEP1
same_genomes("exploration_discussion/ESV6/metabolites_ESV6_closest.csv","exploration_discussion/ESV6/ESV6_metabolites.csv") #check that is different from wero
ESV_to_parse<-creating_df_with_both("exploration_discussion/ESV6/metabolites_ESV6_closest.csv","exploration_discussion/ESV6/ESV6_metabolites.csv")
...
#And saving it all
write.csv(Summary_df, file="exploration_discussion/ESV6/ESV6_specificity.csv")
write.csv(ESV_study_names, file="exploration_discussion/ESV6/ESV6_study_names.csv")
write.csv(ESV_study, file="exploration_discussion/ESV6/ESV6_study.csv")

##############################################################################################################
##############################################################################################################
#Tree for the paper, more information than just one branch for the visual 
ps.clos<-subset_taxa(ps, Class=="c__Gammaproteobacteria")
...
for_plot<-as.matrix(for_plot)
tax_table(ps.clos)<-for_plot
plot_tree(ps.clos,color="enriched", label.tips ="names" , title = "Gammaproteobacteria")
