...
ps.clos<-subset_taxa(ps, Order=="o__Desulfovibrionales")
...
tree_analysis<-plot_tree(ps.clos,color="enriched", label.tips = "Genus.y", title = "Order Desulfovibrionales")

node_of_interets<-tree_analysis$data$V1[tree_analysis$data$OTU == rownames(final_results)[final_results$ESV == "ESV#14"]]
...
to_extract_with_ESV <-tmp[tmp != rownames(final_results)[final_results$ESV == "ESV#14"]]

#this does not have a closest genome available
...
#STEP1
same_genomes("exploration_discussion/ESV14/closest_ESV14_metabolites.csv","exploration_discussion/ESV14/ESV14_metabolites.csv") #check that is different from wero
ESV_to_parse<-creating_df_with_both("exploration_discussion/ESV14/closest_ESV14_metabolites.csv","exploration_discussion/ESV14/ESV14_metabolites.csv")
...
#And saving it all
write.csv(Summary_df, file="exploration_discussion/ESV14/ESV14_specificity.csv")
write.csv(ESV_study_names, file="exploration_discussion/ESV14/ESV14_study_names.csv")
write.csv(ESV_study, file="exploration_discussion/ESV14/ESV14_study.csv")
...
for_plot<-as.matrix(for_plot)
tax_table(ps.clos)<-for_plot
plot_tree(ps.clos,color="enriched", label.tips ="names" , title = "order Desulfovibrionales")
