...
ps.clos<-subset_taxa(ps, Family=="f__Coriobacteriaceae")
...
node_of_interets<-tree_analysis$data$V1[tree_analysis$data$OTU == rownames(final_results)[final_results$ESV == "ESV#21"]]
...
to_extract_with_ESV <-tmp[tmp != rownames(final_results)[final_results$ESV == "ESV#21"]]

###tree bootstrap value not great, can't search more here 
##############################
...
#STEP1
same_genomes("exploration_discussion/ESV21/closest_ESV21_metabolites.csv","exploration_discussion/ESV21/ESV21_metabolites.csv") #check that is different from wero
ESV_to_parse<-creating_df_with_both("exploration_discussion/ESV21/closest_ESV21_metabolites.csv","exploration_discussion/ESV21/ESV21_metabolites.csv")
...
#And saving it all
write.csv(Summary_df, file="exploration_discussion/ESV21/ESV21_specificity.csv")
write.csv(ESV_study_names, file="exploration_discussion/ESV21/ESV21_study_names.csv")
write.csv(ESV_study, file="exploration_discussion/ESV21/ESV21_study.csv")

##############################################################################################################
##############################################################################################################
#Tree for the paper, more information than just one branch for the visual 
ps.clos<-subset_taxa(ps, Family=="f__Bifidobacteriaceae")
...
###not a single one but would have to compare to the whole tree
